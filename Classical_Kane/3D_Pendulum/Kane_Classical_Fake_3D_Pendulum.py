import csv
import time
file_path = "tempi_di_esecuzione_3Dfakependulum_2dof_Inteli7.csv"
tempi = []
...
for i in range(10):
    ...
    Rs_Ao = -m*B1.acc(n).xreplace(gencoordDer_to_speeds_dict).xreplace(gencoordDerDer_to_speedsDer_dict)
    Rs_Bo = -m*B2.acc(n).xreplace(gencoordDer_to_speeds_dict).xreplace(gencoordDerDer_to_speedsDer_dict)
    R_Ao = (m*g*n.x)
    R_Bo = (m*g*n.x)


    Ts_A = -(b1.ang_acc_in(n).dot(I_1) + me.cross(b1.ang_vel_in(n), I_1).dot(b1.ang_vel_in(n))).xreplace(gencoordDer_to_speeds_dict).xreplace(gencoordDerDer_to_speedsDer_dict)
    Ts_B = -(b2.ang_acc_in(n).dot(I_2) + me.cross(b2.ang_vel_in(n), I_2).dot(b2.ang_vel_in(n))).xreplace(gencoordDer_to_speeds_dict).xreplace(gencoordDerDer_to_speedsDer_dict)
    ...
    # print()
    # sy.pprint(sy.simplify(F1s + F1))
    # print()
    # print()
    # sy.pprint(sy.simplify(F2s + F2))
    # print()
    # input()
