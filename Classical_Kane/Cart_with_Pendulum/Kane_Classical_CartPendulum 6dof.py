...
import csv
import time
file_path = "tempi_di_esecuzione_cartpendulum_6dof_AmdRyzen.csv"
tempi = []
...
for i in range(10):
   ...
   m_h = 6                    # Number of constraints present 
   ...
   A.set_pos(O, q1*N.x + a*N.y + q4*N.z)                      # Setting point A relative to the inertial ref. Frame
   ...
   C.orient_body_fixed(N, (q5,0,q6),'zyx')                                     #Defining the cart ref. frame as rotated of 0° wrt the Inertial. By doing so, i tell the code that the two frames have their axes alligned                        
   P.orient_body_fixed(C, (-(sy.pi/2 - q2),q3,0),'zyx')         #Defining the pendulum ref. frame as rotated of q2 wrt to the cart ref. frame
   ...
   v_A_part4 = v_A.diff(u4, N)
   v_A_part5 = v_A.diff(u5, N)
   v_A_part6 = v_A.diff(u6, N)                                                  
   v_G_part1 = v_G.diff(u1, N)
   v_G_part2 = v_G.diff(u2, N) 
   v_G_part3 = v_G.diff(u3, N)    
   v_G_part4 = v_G.diff(u4, N)
   v_G_part5 = v_G.diff(u5, N)
   v_G_part6 = v_G.diff(u6, N)                         
   omega_C_part1 = omega_C.diff(u1, N)
   omega_C_part2 = omega_C.diff(u2, N)
   omega_C_part3 = omega_C.diff(u3, N)
   omega_C_part4 = omega_C.diff(u4, N)
   omega_C_part5 = omega_C.diff(u5, N)
   omega_C_part6 = omega_C.diff(u6, N)
   ...
   omega_P_part5 = omega_P.diff(u5, N)
   omega_P_part6 = omega_P.diff(u6, N)
   ...
   v_A_part = sy.Matrix([[v_A_part1.dot(N.x), v_A_part1.dot(N.y), v_A_part1.dot(N.z)],
                        ...
                        [v_A_part4.dot(N.x), v_A_part4.dot(N.y), v_A_part4.dot(N.z)],
                        [v_A_part5.dot(N.x), v_A_part5.dot(N.y), v_A_part5.dot(N.z)],
                        [v_A_part6.dot(N.x), v_A_part6.dot(N.y), v_A_part6.dot(N.z)]])
   ...
   v_G_part = sy.Matrix([[v_G_part1.dot(N.x), v_G_part1.dot(N.y), v_G_part1.dot(N.z)],
                        ...
                        [v_G_part4.dot(N.x), v_G_part4.dot(N.y), v_G_part4.dot(N.z)],
                        [v_G_part5.dot(N.x), v_G_part5.dot(N.y), v_G_part5.dot(N.z)],
                        [v_G_part6.dot(N.x), v_G_part6.dot(N.y), v_G_part6.dot(N.z)]])
   ...
   omega_C_part = sy.Matrix([[omega_C_part1.dot(N.x), omega_C_part1.dot(N.y), omega_C_part1.dot(N.z)],
                           ...
                           [omega_C_part4.dot(N.x), omega_C_part4.dot(N.y), omega_C_part4.dot(N.z)],
                           [omega_C_part5.dot(N.x), omega_C_part5.dot(N.y), omega_C_part5.dot(N.z)],
                           [omega_C_part6.dot(N.x), omega_C_part6.dot(N.y), omega_C_part6.dot(N.z)]])
   ...
   omega_P_part = sy.Matrix([[omega_P_part1.dot(N.x), omega_P_part1.dot(N.y), omega_P_part1.dot(N.z)],
                           ...
                           [omega_P_part5.dot(N.x), omega_P_part5.dot(N.y), omega_P_part5.dot(N.z)],
                           [omega_P_part6.dot(N.x), omega_P_part6.dot(N.y), omega_P_part6.dot(N.z)]])
