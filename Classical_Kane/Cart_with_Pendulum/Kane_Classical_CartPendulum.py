...
# Uncommenting the for cycle makes run the code for 10 times, saving the performance time of each run in a csv file
# import csv
# import time
# file_path = "tempi_di_esecuzione_cartpendulum_2dof_Inteli7.csv"
# tempi = []
...
m_h = 10                   # Number of constraints present
m_nh = 0                   # Number of Non-Holonomic constrains present

# Calculations
# System's Degree of freedom, tells us the n° of Gen. Coordinates we will have
dof = 6*n - m_h
...
w = sy.symbols('h')                # Box width [m] Cart center of mass's height wrt x axis [m]
a = sy.symbols('a')
...
# Generalized inertia forces vertical vector
F = sy.Matrix.zeros(dof, 1)
...
for i in range(dof):
    ...
    F[i, 0] = - (Md[0] * a[0, 0:3].dot(v_A_part[i, 0:3]) + ((omegadot[0, 0:3]*I_m1) + omega[0, 0:3].cross(I_m1*(omega[0, 0:3].T))).dot(omega_C_part[i, 0:3])) - \
                (Md[1] * a[1, 0:3].dot(v_G_part[i, 0:3]) + ((omegadot[1, 0:3]*I_m2) + omega[1, 0:3].cross(I_m2*(omega[1, 0:3].T))).dot(omega_P_part[i, 0:3]))

    # Generalized Active forces
    f[i, 0] = (Ftot[0, 0:3].dot(v_A_part[i, 0:3]) + (Torqtot[0, 0:3]).dot(omega_C_part[i, 0:3])) + \
              (Ftot[1, 0:3].dot(v_G_part[i, 0:3]) + (Torqtot[1, 0:3]).dot(omega_P_part[i, 0:3]))
...
# -------------------------------------------------------------------------------------
# # Uncommenting this paragraph permit to save the times required for each code run, evaluating its performances
