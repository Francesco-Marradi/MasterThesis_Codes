...
import csv
import time
file_path = "tempi_di_esecuzione_doublependulum_6dof_AmdRyzen.csv"
tempi = []
...
for i in range(10):
   ...
   O.set_pos(O, q6*P1.x + q5*P1.y + 0*P1.z)                     # Setting point O as the origin of the pendulum 1 ref. frame
   ...
   v_P_part5 = v_P.diff(u5, N)
   v_P_part6 = v_P.diff(u6, N)                                              
   v_Q_part1 = v_Q.diff(u1, N)
   v_Q_part2 = v_Q.diff(u2, N)
   v_Q_part3 = v_Q.diff(u3, N) 
   v_Q_part4 = v_Q.diff(u4, N)   
   v_Q_part5 = v_Q.diff(u5, N)
   v_Q_part6 = v_Q.diff(u6, N)                           
   ...
   omega_Q_part5 = omega_Q.diff(u5, N)
   omega_Q_part6 = omega_Q.diff(u6, N)
   ...
   v_A_part = sy.Matrix([[v_P_part1.dot(N.x), v_P_part1.dot(N.y), v_P_part1.dot(N.z)],
                        ...
                        [v_P_part5.dot(N.x), v_P_part5.dot(N.y), v_P_part5.dot(N.z)],
                        [v_P_part6.dot(N.x), v_P_part6.dot(N.y), v_P_part6.dot(N.z)]])
   ...
   v_G_part = sy.Matrix([[v_Q_part1.dot(N.x), v_Q_part1.dot(N.y), v_Q_part1.dot(N.z)],
                        ...
                        [v_Q_part5.dot(N.x), v_Q_part5.dot(N.y), v_Q_part5.dot(N.z)],
                        [v_Q_part6.dot(N.x), v_Q_part6.dot(N.y), v_Q_part6.dot(N.z)]])
   ...
   omega_P_part = sy.Matrix([[omega_Q_part1.dot(N.x), omega_Q_part1.dot(N.y), omega_Q_part1.dot(N.z)],
                           ...
                           [omega_Q_part5.dot(N.x), omega_Q_part5.dot(N.y), omega_Q_part5.dot(N.z)],
                           [omega_Q_part6.dot(N.x), omega_Q_part6.dot(N.y), omega_Q_part6.dot(N.z)]])
