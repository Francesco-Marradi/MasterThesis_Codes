...
#    # Pezzettino di funzione che ha il compito di espandere i termini troppo lunghi
#     expanded_terms = []
#     for term in terms:
#         if len(str(term)) > 2500:
#             expanded_terms.extend(sy.Add.make_args(term.expand()))
#         else:
        ...
        try:
            for idx, chunk in enumerate(chunks, start=1):
                ...
                if chunk_key in simplification_cache:
                    ...
                    simplified_chunk = simplify_with_timeout(iteration, chunk, i, j, queue, timeout=300)
                    simplification_cache[chunk_key] = simplified_chunk

                simplified_chunks.append(simplified_chunk)

        except TimeoutError:
            # print(f"⏱️ Timeout nella semplificazione della cella [{i},{j}] — Cella flaggata.")
            raise TimeoutError((i, j))
...
# FUNZIONE VECCHIA
# def simplify_with_timeout(iteration, expr, i, j, queue, timeout):
#     """
#     Semplifica expr entro timeout secondi usando un ThreadPool locale. Se la semplificazione non riesce,
#     restituisce il chunk originale.
#     """
#     # if queue:
#     #     queue.put(((i, j), "SIMPLIFYING"))

#     def worker(expr_, iteration_):
#         if iteration_ == 1:
#             return sy.simplify(expr_)
#             # return sy.trigsimp(expr_)
#         else:
#             # return sy.simplify(expr_)
#             return sy.trigsimp(expr_)

#     with ThreadPoolExecutor(max_workers=1) as executor:
#         future = executor.submit(worker, expr, iteration)
#         try:
#             return future.result(timeout=timeout)
#         except TimeoutError:
#             raise TimeoutError("Timeout durante la semplificazione")


def simplify_chunk(args):
    """
    funzione di alto livello per poter "bloccare" il processo, nel caso in cui il timeout scadesse
    """
    iteration, chunk = args
    if iteration == 1:
...
def simplify_with_timeout(iteration, chunk, i, j, queue, timeout, cache=None):
    """
    Semplifica expr entro timeout secondi usando un ThreadPool locale. Se la semplificazione non riesce,
    restituisce il chunk originale.
    """
    with ProcessPoolExecutor(max_workers=1) as executor:
        future = executor.submit(simplify_chunk, (iteration, chunk))
        try:
            return future.result(timeout=timeout)
        except TimeoutError:
